...
public class Player1StatePanel extends JApplet implements ActionListener{
	...
	// コンストラクタ
	public Player1StatePanel(AppletContext ac1, Dimension size1, MainPanel mp1)
	{
		pane = getContentPane();
		...
		//キャラクターパネル初期化
		p1csp = new Player1CharacterStatusPanel[my_unit.length];
		for (int i = 0; i < my_unit.length; i++) {
			p1csp[i] = new Player1CharacterStatusPanel(ac, size, mp, this, my_unit[i],i);
		}
		...
		for(int i = 0; i < panel.length; i++){
			...
			panel[i].add(p1csp[i]);
			p1csp[i].setBounds(25, 0, 150, 300);
		}
	...
	public void detailFrameHide(int n){
		p1csp[n].detailFrameHide();
	}

	public void weaponSelectFrameHide(int n){
		p1csp[n].weaponSelectFrameHide();
	}
